...
DATABASE_LOCATION = "sqlite:///tracks_analytics.sqlite"

if __name__ == "__main__":

#Importing the songs_df from the Extract.py
    load_df=Extract.get_listen_history()
    if(Transform.Data_Quality(load_df) == False):
        raise ("Failed at Data Validation")
    
    metric_df=Extract.get_song_metrics(load_df)
    
    print(metric_df)
    
    Transformed_df=Transform.Transform_df(load_df)
    profile_df=Extract.get_user_profile()
